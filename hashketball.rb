...
def num_points_scored(player)
  game_hash[:home][:players].each do |key, elem|
    if key == player
      return game_hash[:home][:players][player][:points]
    end
  end
  game_hash[:away][:players].each do |key, elem|
    if key == player
...
def big_shoe_rebounds
  max = game_hash[:home][:players].first[1][:shoe]
  rebounds = ""
  game_hash[:home][:players].each do |player, value|
    if game_hash[:home][:players][player][:shoe] >= max
